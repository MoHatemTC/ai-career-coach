...
class ReadinessScore(SQLModel, table=True):
    ...
    job_match_id:
        Foreign key referencing ``job_matches.id``, if this score was generated
        by deepening an existing match. Null for standalone assessments.
    overall_score:
    ...
    benchmark_id: int = Field(
        foreign_key="role_benchmarks.id",
        index=True,
    ...
    job_match_id: Optional[int] = Field(
        default=None,
        foreign_key="job_matches.id",
        index=True,
        description="FK → job_matches.id, if this score was generated by deepening an existing match. Null for standalone assessments.",
    )
